...
%       New: Bound the biomass to its optimal value and then overlay
        FBAsol_alt = optimizeCbModel(model_alt,'max');
%         model_alt = changeRxnBounds(model_alt,model_alt.rxns{1799},mu,'l');
        model_alt = changeRxnBounds(model_alt,model_alt.rxns{1799},0.999*FBAsol_alt.f,'l');
...
%             FBAsol_alt = optimizeCbModel(model_alt,'max');
            model_alt = changeRxnBounds(model_alt,model_alt.rxns{1799},mu,'l');
            models{i} = model_alt; % Save model in the map
        end
...
model_alt = changeRxnBounds(model_alt,model_alt.rxns{1799},0.999*FBAsol_alt.f,'l');
% model_alt = changeRxnBounds(model_alt,model_alt.rxns{1799},mu,'l');
[FBAsol_alt,model_alt] = overlayMultiomicsData(model_alt,data,100,waiverList);
...
subplot(1,3,1);
plot(1:lastIt,reshape(mean(record_obj(1,:,:)),[1,lastIt]),1:lastIt,record_gBestObj);
...
subplot(1,3,3);
scatter(X,Y);
title('Predicted proteome vs transcriptome');
xlabel('predicted');
ylabel('transcriptome');

clear sm X Y lbl cplxIdx rxnIdx mn i j k;

% Comparing KS Scale from 1 dataset and 2 datasets
% X = [];
% Y = [];
% lbl = {};
% 
% for i = 1:length(fullCplx)
%     if variedKeff(i)
%         X(end+1,1) = record_gBest(i);
%         Y(end+1,1) = record_gBest_multi(i);
%         lbl{end+1} = fullCplx{i};
%     end
% end
% 
% figure;
% scatter(X,Y);
% xlabel('Keff Scale from only Pi rep Set');
% ylabel('Keff Scale from both Pi rep and Pi dep Set');
% set(gca,'YScale','log');
% set(gca,'XScale','log');
% text(X,Y,lbl);
